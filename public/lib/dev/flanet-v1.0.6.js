	...
	this.animation = new Kinetic.Animation(function(frame){
		...
		step : function(){
			for(var i = 0; i < this.worlds.length; i++){
				var w = this.worlds[i];
				if(w.getActivation()){
					w.step(this.timestep, this.iteration);
	...
	Flanet.World.prototype = {
		...
		wakeUp : function(){
			for(var i = 0; i < this.panels.length; i++){
				...
				this.panels[i].getBody().SetLinearVelocity(v);
				this.panels[i].getBody().WakeUp();
			}
			this.start();
		},
		...
		step : function(timeStep, iteration){
			if(this.activation){
				for(var i = 0; i < this.panels.length; i++){
					var body = this.panels[i].getBody();
					...
					var scala = b2Math.MulFV(body.m_mass * 0.01 *
						this.getAltitute(this.panels[i].getPosition()), direction);
		...
		getAvgSpeed : function(){
			var s = 0;
			for(var i = 0; i < this.panels.length; i++){
				var v = this.panels[i].getVelocity();
